//Source code obtained and modifierd from https://github.com/pj4533/dhtxx
import Glibc
import Foundation
import SwiftyGPIO
...
public class DHT {
	...
	func binaryStringForInt(number: Int) -> String {
	 ...
	 func binaryStringForUInt8(number: UInt8) -> String {
	 	return binaryStringForInt(number: Int(number))
	 }


	func read(debug: Bool = false) -> (temperature: Double, humidity: Double) {
		...
		guard highpulse >= 40 else { return (0,0) }
  ...
  // 		if debug {
		// 	print("Computed checksum: \(self.binaryStringForInt(number: computedChecksum))(\(computedChecksum)) = (\(self.binaryStringForInt(number: data[0]))(\(data[0])) + \(self.binaryStringForInt(number: data[1]))(\(data[1])) + \(self.binaryStringForInt(number: data[2]))(\(data[2])) + \(self.binaryStringForInt(number: data[3]))(\(data[3]))) & 0xFF")
		// 	print("  Actual checksum: \(self.binaryStringForUInt8(number: data[4]))(\(data[4]))")
		// }
